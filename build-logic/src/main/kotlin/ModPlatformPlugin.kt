...
abstract class ModPlatformPlugin @Inject constructor() : Plugin<Project> {
	...
	private fun Project.configureProcessResources(
		...
		tasks.named<ProcessResources>("processResources") {
			...
			var issuesUrl = prop("mod.issues_url").trim()
			if (issuesUrl.isEmpty()) {
				val sourcesUrl = prop("mod.sources_url").trim()
				val normalizedSources = sourcesUrl.trimEnd('/')
				issuesUrl = if (normalizedSources.isEmpty()) "" else "$normalizedSources/issues"
			}
			if (issuesUrl.isNotEmpty()) {
				val hasScheme = Regex("^[a-zA-Z][a-zA-Z0-9+.-]*:").containsMatchIn(issuesUrl)
				if (!hasScheme) {
					issuesUrl = when {
						issuesUrl.startsWith("//") -> "https:$issuesUrl"
						issuesUrl.startsWith("/") -> ""
						else -> "https://$issuesUrl"
					}
				}
			}
			...
			val props = mapOf(
				...
				"fabric_loader" to prop("deps.fabric-loader"),
				"fabric_api" to prop("deps.fabric-api"),
				"modmenu" to prop("deps.modmenu"),
				"dependencies" to dependencies
			)
			val stripIssueTrackerLine: (String) -> String = { line ->
				if (issuesUrl.isEmpty() && line.trimStart().startsWith("issueTrackerURL")) "" else line
			}
			...
			when {
				...
				isNeoForge -> {
					fun versionBefore(current: String, target: String): Boolean {
					...
					val legacyModsToml = versionBefore(mcVersion, "1.20.6")
					if (legacyModsToml) {
						filesMatching("META-INF/mods.toml") {
							expand(props)
							if (issuesUrl.isEmpty()) {
								filter(stripIssueTrackerLine)
							}
						}
